...
class MyDense(tf.keras.layers.Layer):
  ...
  def build(self, shape_input):
    self.kernel = self.add_variable("kernel", shape=[shape_input[-1].value, self.shape_output])
    self.bias = self.add_variable("bias", shape = [self.shape_output])
...
# Train
epoch = 100
runlog = open("run_mnistlrdecay.log", "w")
with tf.Session() as sess:
  ...
  for i in range(epoch):
    ...
    for offset in range(0, num_example, BATCHSIZE):
      ...
      if batch_j % 10 == 0:
        print("[TRAIN] Epoch {}, Batch {}: lr = {:.3f}, loss = {:.3f}, accuracy = {:.3f}".format(i+1, batch_j, lr, train_loss, train_accuracy))
